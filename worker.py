...
class Worker:
    ...
    with sess.as_default(), sess.graph.as_default():
      ...
      try:
        while not coord.should_stop():
          ...
          if self.name == "worker_0" and self.steps % 10000 == 0 :
              self.eval_policy(sess)
  ...
  def load_model(self, sess):
      self.saver.restore(sess, "models/model80000/model.ckpt")
      print("loaded model")

  def save_model(self, sess):
      path_to_model_pb = "models/model{}".format(self.steps)
      os.makedirs(path_to_model_pb)
      print("save model to ", path_to_model_pb)
      save_path = self.saver.save(sess, path_to_model_pb + "/model.ckpt")
      #tf.train.write_graph(self.policy_net.as_graph_def(), path_to_model_pb, 'saved_model.pb', as_text=False)
  
  def eval_policy(self, sess):
    ...
    for _ in range(n):
      ...
      if done:
        ...
        if len(self.returns_list) > 0 and len(self.returns_list) % 100 == 0:
          mean = np.mean(self.returns_list[-100:])
          s = len(self.returns_list)
          self.writer.add_scalar('reward', mean, s)
